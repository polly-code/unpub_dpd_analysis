...
int main(int argc, char *argv[])
{
    ...
    for (unsigned int i = 0; i < argc; i++)
    {
        if (rank == 0 && (strcmp(argv[i], "-h") == 0 || strcmp(argv[i], "--help") == 0))
        {
            cout << "Program to calculate MSD using lammpstrj file.\n"
                 ...
                 << "\t-lmp, takes lammpstrj format for input data\n"
                 << "-n, --noise, add systematic error\n"
                 << "-w, -write, write three tracks to calculate MSD" << endl;
            ierr = 3;
            MPI_Finalize();
            return 3;
        }
        ...
        if (strcmp(argv[i], "-xyz") == 0)
            xyztrj = true;
        if (strcmp(argv[i], "-n") == 0 || strcmp(argv[i], "--noise") == 0)
        {
            noise = atof(argv[i + 1]);
        }
    ...
    if (write_tracks)
    {
        if (rank == 0)
        {
            if (aver)
            {
                cout << "Writing tracks for average MSD calculation" << endl;
            }
            else
            {
                cout << "Writing tracks for single MSD calculation" << endl;
            }
            tracks_out.open(mypath + "_tracks.dat", ios::out | ios::app);
            tracks_out << "id,x,y,z,t,dx,dy,dz" << endl;
            tracks_out.close();
        }
    }
        ...
        while (getline(input_file, emp))
        {
            ...
            for (int i = 0; i < numberBeads; i++)
            {
                ...
                if (id >= bead_begin && id < bead_end)
                {
                    if (noise != 0)
                    {
                        input_file >> emp >> x[id][frame] >> y[id][frame] >> z[id][frame];
                        dx = (((double)rand() / (RAND_MAX)) * 2 - 1) * noise;
                        dy = (((double)rand() / (RAND_MAX)) * 2 - 1) * noise;
                        dz = (((double)rand() / (RAND_MAX)) * 2 - 1) * noise;
                        if (write_tracks)
                        {
                            //ec = MPI_File_write_at(fh, (frame * numberBeads + id) * 3 * sizeof(float), &x[id][frame], 1, MPI_FLOAT, MPI_STATUS_IGNORE);
                            tracks_out.open(mypath + "_tracks.dat", ios::out | ios::app);
                            tracks_out << id + 1 << ',' << x[id][frame] << ',' << y[id][frame] << ',' << z[id][frame] << ','
                                       << frame << ',' << dx << ',' << dy << ',' << dz << endl;
                            tracks_out.close();
                        }
                        x[id][frame] += dx;
