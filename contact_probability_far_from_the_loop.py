...
out_range3 = [[0, 350], [0, 350]]
out_tad3 = calc_pc(cm, out_range3)
out_range4 = [[651, 1000], [651, 1000]]
out_tad4 = calc_pc(cm, out_range4)
out_tad_far = np.mean([out_tad3, out_tad4], axis=0)
out_tad_far[out_tad_far == 0] = np.nan
out_tad_far_x, out_tad_far_y = logbin(
    np.arange(1, len(out_tad_far)), out_tad_far[1:], numdots=30
)
...
hic_pc = pd.read_csv("../bonev_track/Bonev.scaling.1kb.csv")
hic_x, hic_y = logbin(
...
plt.plot(out_tad_far_x, out_tad_far_y / max(out_tad_far_y), label="out, far from CTCFs")
...
plt.xscale("log")
plt.yscale("log")
plt.savefig(sys.argv[1] + "pc_contact_prob_w_out_far.png", dpi=300)
plt.close()
