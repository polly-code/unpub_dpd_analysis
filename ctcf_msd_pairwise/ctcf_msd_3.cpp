...
int main(int argc, char *argv[])
{
    ...
    int numberBeads, numberFrames, scale_factor, skip_rows;
    string emp, line;
    fstream in;
    ...
    if (rank == 0)
    {
        ...
        in.close();
        if (scale_factor != 1)
            numberFrames = numberFrames / scale_factor + 1;
    ...
    while (getline(in, emp))
    {
        ...
        if (line_number % scale_factor == 0)
        {
            ...
            {
                if (rank == i)
                    in >> ctcf_dst[line_number / scale_factor];
                else
                    in >> emp;
            }
        }
